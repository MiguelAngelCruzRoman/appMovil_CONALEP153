...
public class MensajesGrupoAdapter extends RecyclerView.Adapter<MensajeHolder> {
    private List<Mensaje> listaMensajes = new ArrayList<>();
    private Context c;

    public MensajesGrupoAdapter(Context c) {
        this.c = c;
    }
    ...
    public MensajeHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(c).inflate(R.layout.mensaje_grupo_list_item, parent, false);
    ...
    public void onBindViewHolder(@NonNull MensajeHolder holder, int position) {
        ...
        if (mensaje.getIdUsuario().equals(idUsuarioActual)) {
            ...
            holder.getNombreUsuario().setScaleX(-1);
        } else {
            holder.itemView.setScaleX(1);
            holder.getFechaHoraMensaje().setScaleX(1);
            holder.getContenidoMensaje().setScaleX(1);
            holder.getNombreUsuario().setScaleX(1);
            String colorHex = mensaje.colorBordeTarjeta();
